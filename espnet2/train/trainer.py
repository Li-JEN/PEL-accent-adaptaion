...
class Trainer:
    ...
    def train_one_epoch(
        ...
        for iiter, (utt_id, batch) in enumerate(
            ...
            with autocast(scaler is not None):
                ...
                loss /= accum_grad
            if "pair" in batch:
                if schedulers[0].last_epoch <= options.add_loss_step:
                    stats = {k: v for k, v in stats.items() if k != "wd_loss"}
                    reporter.register(stats, weight)
                else:
                    # emp_value = min(3e-3, schedulers[0].last_epoch / 400 * 1e-3)
                    # emp_value = 1e-3 #for wd
                    # emp_values = np.linspace(0.000, 2e-3,1200)
                    # emp_value = emp_values[schedulers[0].last_epoch-800]
                    loss -= stats["wd_loss"]
                    stats["loss"] -= stats["wd_loss"]
                    reporter.register(stats, weight)
            else:
                stats = {k: v for k, v in stats.items() if k != "wd_loss"}
                reporter.register(stats, weight)
    ...
    def validate_one_epoch(
        ...
        for (utt_id, batch) in iterator:
            ...
            # if schedulers[0].last_epoch <= 600:
            #     stats = {k: v for k, v in stats.items() if k != "wd_loss"}
            #     reporter.register(stats, weight)
            # else:
            #     emp_value = schedulers[0].last_epoch / 600 
            #     stats["wd_loss"] *= emp_value
            #     reporter.register(stats, weight)
            # reporter.next()
            if "pair" in batch:
                if schedulers[0].last_epoch <= options.add_loss_step:
                    ...
                    # emp_value = min(1e-2,schedulers[0].last_epoch / 800 * 1.5e-3)
                    # emp_value = min(3e-3, schedulers[0].last_epoch / 400 * 1e-3)
                    # emp_value = 1e-3 #for wd
                    # emp_value = 1e-1 #for mmd
