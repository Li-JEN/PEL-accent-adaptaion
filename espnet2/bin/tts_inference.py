...
def inference(
    ...
    with NpyScpWriter(
        ...
        output_dir / "latent", output_dir / "latent/feats.scp"
    ) as latent_writer, open(
        ...
        for idx, (keys, batch) in enumerate(loader, 1):
            ...
            if output_dict.get("latent_feats") is not None:
                print(len(output_dict["latent_feats"]))
                latent_writer[key] = np.array([each.cpu().numpy() for each in output_dict["latent_feats"]])
