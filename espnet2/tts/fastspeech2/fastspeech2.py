...
from espnet.nets.pytorch_backend.conformer.encoder import ReprogrammingLayer
...
class FastSpeech2(AbsTTS):
    ...
    def __init__(
        ...
        encoder_use_reprogram: bool = False,
        decoder_use_reprogram: bool = False,
        intermediate_use_reprogram: bool = False,
        use_adapter:bool = False,
        latent_use_reprogram: bool = False,
        use_mmd= False,
        use_swd= False,
        use_l2= False
    ):
        ...
        if encoder_type == "transformer":
            ...
            self.encoder = ConformerEncoder(
                ...
                zero_triu=zero_triu,
                use_reprogram=encoder_use_reprogram,
                reprogram_token = 50
        ...
        self.intermediate_use_reprogram = intermediate_use_reprogram
        if intermediate_use_reprogram:
            self.pitch_reprogram = ReprogrammingLayer(adim,100)
            self.energy_reprogram = ReprogrammingLayer(adim,100)
            self.duration_reprogram = ReprogrammingLayer(adim,100)
        # define GST
        ...
        if decoder_type == "transformer":
            ...
            self.decoder = ConformerEncoder(
                ...
                use_reprogram=decoder_use_reprogram,
                reprogram_token = 714, # Max len from GT
                use_adapter = use_adapter,
                use_latent_reprogram = latent_use_reprogram
            )
        else:
    ...
    def forward(
        ...
        pair_lengths: Optional[torch.Tensor] = None,
    ...
    def _forward(
        ...
        if self.stop_gradient_from_pitch_predictor:
            if self.intermediate_use_reprogram:
                p_hs = self.pitch_reprogram(hs.detach())
                p_outs = self.pitch_predictor(p_hs, d_masks.unsqueeze(-1))
            else:
                p_outs = self.pitch_predictor(hs.detach(), d_masks.unsqueeze(-1))
        else:
            if self.intermediate_use_reprogram:
                p_hs = self.pitch_reprogram(hs)
                p_outs = self.pitch_predictor(p_hs, d_masks.unsqueeze(-1))
            else:
                p_outs = self.pitch_predictor(hs, d_masks.unsqueeze(-1))
        if self.stop_gradient_from_energy_predictor:
            if self.intermediate_use_reprogram:
                e_hs = self.energy_reprogram(hs.detach())
                e_outs = self.energy_predictor(p_hs, d_masks.unsqueeze(-1))
            else:
                e_outs = self.energy_predictor(hs.detach(), d_masks.unsqueeze(-1))
        else:
            if self.intermediate_use_reprogram:
                e_hs = self.energy_reprogram(hs)
                e_outs = self.energy_predictor(e_hs, d_masks.unsqueeze(-1))
            else:
                e_outs = self.energy_predictor(hs, d_masks.unsqueeze(-1))

        if is_inference:
            if self.intermediate_use_reprogram:
                d_hs = self.duration_reprogram(hs)
            else:
                d_hs = hs
            d_outs = self.duration_predictor.inference(d_hs, d_masks)  # (B, T_text)
            ...
            if self.intermediate_use_reprogram:
                d_hs = self.duration_reprogram(hs)
                d_outs = self.duration_predictor(d_hs, d_masks)
            else:
