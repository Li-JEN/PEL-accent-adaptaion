...
Options:
    ...
    --freeze_param       # freeze_param for fine-tuning(default="${freeze_param}")
EOF
)
...
if ! "${skip_train}"; then
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        _pair_feats_dir=/home/espnet/egs2/csmsc/tts1/exp/tts_stats_raw_phn_pypinyin_g2p_phone/csmsc
        _opts+="--train_data_path_and_name_and_type ${_pair_feats_dir}/csmsc_tr_no_dev/latent/feats.scp,pair,npy "
        _opts+="--valid_data_path_and_name_and_type ${_pair_feats_dir}/csmsc_dev/feats.scp,pair,npy "
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        if [ -z "${teacher_dumpdir}" ]; then
            ...
            _pair_feats_dir=/home/espnet/egs2/csmsc/tts1/exp/tts_stats_raw_phn_pypinyin_g2p_phone/csmsc
            _opts+="--train_data_path_and_name_and_type ${_pair_feats_dir}/csmsc_tr_no_dev/latent/feats.scp,pair,npy "
            _opts+="--valid_data_path_and_name_and_type ${_pair_feats_dir}/csmsc_dev/latent/feats.scp,pair,npy "
        ...
        if [ -n "${freeze_param}" ]; then
            _opts+="--freeze_param ${freeze_param}"
        fi
...
git checkout $(git show -s --format=%H)"
     ...
     download_model   # Generate description file
